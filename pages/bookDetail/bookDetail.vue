<template>
	<view class="geryTheme">
		...
		<view class="flex align-center py-2 cal" style="height: 250rpx;">
			<image :src="currentBook.imgurl" mode="aspectFill" lazy-load class="flex-1 mx-2 rounded"
				style="width: 250rpx;height: 210rpx;"></image>
				...
				<swiper-item>
					<scroll-view scroll-y :style="{height: `${calHeight}rpx`}">
						<block v-for="item in chapterCatalog" :key="item.id">
							<view hover-class="bg-light" class="p-2 text-ellipsis border-bottom"
								@tap="toReadingPage(item.id)"> {{item.title}} </view>
						</block>
					</scroll-view>
				</swiper-item>
			</swiper>
...
<script>
	...
	export default {
		...
		async onLoad(e) {
			...
			let {
				chapterCatalog
			} = await this.$http.get('/testSynopsis');
